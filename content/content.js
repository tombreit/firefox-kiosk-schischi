function createPanel() {
    ...
    button_home.onclick = () => {
        if (browser && browser.runtime && browser.runtime.sendMessage) {
            browser.runtime.sendMessage({action: "getHomepage"})
                .then(url => { window.location.href = url || "about:home"; })
                .catch(() => { window.location.href = "about:home"; });
        } else {
    ...
    // gray out back/forward and update URL bar on navigation
    function updateNavState(destUrl) {
        button_back.disabled = !navigation.canGoBack;
        button_next.disabled = !navigation.canGoForward;
